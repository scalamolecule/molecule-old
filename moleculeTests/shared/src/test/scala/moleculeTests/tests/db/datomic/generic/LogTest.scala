  ...
  lazy val tests = Tests {
    ...
    "Args" - core { implicit conn =>
      for {
        ...
        _ <- Log(Some("start")).t.get
          .map(_ ==> "Unexpected success 1").recover { case MoleculeException(err, _) =>
          err ==> "Args to Log can only be t, tx or txInstant of type Int/Long/Date. " +
            "Found `start` of type class java.lang.String"
        }

        _ <- Log(Some(tx1), Some("end")).t.get
          .map(_ ==> "Unexpected success 2").recover { case MoleculeException(err, _) =>
          err ==> "Args to Log can only be t, tx or txInstant of type Int/Long/Date. " +
            "Found `end` of type class java.lang.String"
        }

        _ <- Log(Some("42"), Some("45")).t.get
          .map(_ ==> "Unexpected success 3").recover { case MoleculeException(err, _) =>
          err ==> "Args to Log can only be t, tx or txInstant of type Int/Long/Date. " +
