...
object JsonNested2 extends AsyncTestSuite {
  ...
  lazy val tests = Tests {
    ...
    "20" - core { implicit conn =>
      for {

        // Attributes after nested ------------------------------------------

        _ <- Ns.str.bool.Refs1.*(Ref1.int1) insert List(
          ("a", true, List(10, 11))
        )
        // todo: fix insertion of post attribute values
        //        _ <- Ns.str.Refs1.*(Ref1.int1).bool insert List(
        //          ("a", List(10, 11), true)
        //        )

        _ <- Ns.str.Refs1.*(Ref1.int1).bool.getJson.map(_ ==>
          """{
            |  "data": {
            |    "Ns": [
            |      {
        ...
        //        // todo?: post attributes with references?
        //        _ <- Ns.str.Refs1.*(Ref1.int1).bool.Ref1.int1 insert List(
        //          ("a", List(10, 11), true, 100)
        //        )
        //        _ <- Ns.str.Refs1.*(Ref1.int1).bool.getJson.map(_ ==>
        //          """{
        //            |  "data": {
