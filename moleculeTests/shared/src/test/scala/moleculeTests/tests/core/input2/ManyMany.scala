...
object ManyMany extends AsyncTestSuite {
  ...
  lazy val tests = Tests {
    ...
    //            im((in1a, in2a), (in1b, in2b)).get.map(_.sorted ==> (out1a.intersect(out2a) ++ out1b.intersect(out2b)).distinct.sorted)
    //          } catch {
    //            case e: Throwable =>
    //              val expected =
    //                s"""
    ...
    //          try {
    //            im.apply(in1, in2).get.map(_.sorted ==> out1.intersect(out2).sorted)
    //          } catch {
    //            case e: Throwable =>
    //              val expected =
    //                s"""
