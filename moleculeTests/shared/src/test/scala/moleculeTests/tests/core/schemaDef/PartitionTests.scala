package moleculeTests.tests.core.schemaDef
...
object PartitionTests extends AsyncTestSuite {
  ...
  lazy val tests = Tests {
    ...
    //for{
    //    _ <- lit_Book.title("A good book").cat("good").Author.name("Marc").gender("male").inspectSave
...
================================================================================================================
    ...
    "Nested 2 levels" - partition { implicit conn =>
      for {
        ...
        _ <- m(lit_Book.title.Reviewers.name.Professions * gen_Profession.name).get === List(("book", "Jan", List("Musician")))
        // Same as
        _ <- m(lit_Book.title.Reviewers.Professions * gen_Profession.name).get === List(("book", List("Musician")))
      } yield ()
    }
    ...
    "Nested 2 levels + many reference" - partition { implicit conn =>
      for {
        ...
        _ <- m(lit_Book.title.Reviewers * gen_Person.Professions.name).get === List(("book", List("Hacker", "Magician")))
        // Same as
        _ <- m(lit_Book.title.Reviewers.Professions * gen_Profession.name).get === List(("book", List("Hacker", "Magician")))
      } yield ()
    }
    ...
    "Back ref" - {
      ...
      "Adjacent" - partition { implicit conn =>
        for {
          ...
          _ <- m(lit_Book.title.Author.name._lit_Book.Reviewers.name).get === List(("book", "John", "Marc"))
          _ <- m(lit_Book.title.Author.name._lit_Book.Reviewers * gen_Person.name).get === List(("book", "John", List("Marc")))
        } yield ()
      }
      ...
      "Nested + adjacent" - partition { implicit conn =>
        for {
          ...
          _ <- m(lit_Book.title.Author.name._lit_Book.Reviewers.name.Professions.name).get === List(("book", "John", "Marc", "Musician"))
          _ <- m(lit_Book.title.Author.name._lit_Book.Reviewers * gen_Person.name.Professions.name).get === List(("book", "John", List(("Marc", "Musician"))))
        } yield ()
      }
      ...
      "Nested + nested" - partition { implicit conn =>
        for {
          ...
          _ <- m(lit_Book.title.Author.name._lit_Book.Reviewers.name.Professions.name).get === List(("book", "John", "Marc", "Musician"))
          _ <- m(lit_Book.title.Author.name._lit_Book.Reviewers * (gen_Person.name.Professions * gen_Profession.name)).get === List(("book", "John", List(("Marc", List("Musician")))))
        } yield ()
      }
    }
  }
