    ...
    "Card-one variables" - {
      ...
      "apply" - core { implicit conn =>
        for {
          ...
          _ <- Ns(eid).long(long2, long3).update
            .map(_ ==> "Unexpected success").recover { case VerifyModelException(err) =>
            err ==> "[noConflictingCardOneValues]  Can't update multiple values for cardinality-one attribute:" +
              s"\n  Ns ... long($long2, $long3)"
          }
    ...
    "Card-many variables" - {
      ...
      "replace" - core { implicit conn =>
        for {
          ...
          _ <- Ns(eid).longs.replace(long7 -> long8, long8 -> other8).update
          ...
          _ <- Ns(eid).longs.replace(Seq(long7 -> long8, long8 -> other8)).update
            .map(_ ==> "Unexpected success").recover { case Model2TransactionException(err) =>
            err ==> "[valueStmts:default]  Can't replace with duplicate new values of attribute `:Ns/longs`:" +
              "\n8"
          }
