  ...
  lazy val tests = Tests {
    ...
    "Card one" - {
      ...
      "Mandatory" - {
        ...
        "Eq" - core { implicit conn =>
          val inputMolecule = m(Ns.bigInt(?).a1)
          for {
            _ <- oneData
            ...
            _ <- inputMolecule(List(bigInt1, bigInt2)).get.map(_ ==> List(bigInt1, bigInt2))

            // Varargs
            _ <- inputMolecule(bigInt1).get.map(_ ==> List(bigInt1))
            _ <- inputMolecule(bigInt1, bigInt2).get.map(_ ==> List(bigInt1, bigInt2))

            // `or`
            _ <- inputMolecule(bigInt1 or bigInt2).get.map(_ ==> List(bigInt1, bigInt2))
          } yield ()
        }

        "not" - core { implicit conn =>
          val inputMolecule = m(Ns.bigInt.not(?).a1)
          for {
            _ <- oneData
            _ <- inputMolecule(Nil).get.map(_ ==> List(bigInt1, bigInt2, bigInt3))

            _ <- inputMolecule(List(bigInt1)).get.map(_ ==> List(bigInt2, bigInt3))
            _ <- inputMolecule(List(bigInt1, bigInt1)).get.map(_ ==> List(bigInt2, bigInt3))
            _ <- inputMolecule(List(bigInt1, bigInt2)).get.map(_ ==> List(bigInt3))
          } yield ()
        }
        ...
        ">" - core { implicit conn =>
          ...
          for {
            _ <- oneData
            _ <- inputMolecule(Nil).get.map(_ ==> List(bigInt1, bigInt2, bigInt3))
            _ <- inputMolecule(List(bigInt2)).get.map(_ ==> List(bigInt3))
        ...
        ">=" - core { implicit conn =>
          ...
          for {
            _ <- oneData
            _ <- inputMolecule(Nil).get.map(_ ==> List(bigInt1, bigInt2, bigInt3))
            _ <- inputMolecule(List(bigInt2)).get.map(_ ==> List(bigInt2, bigInt3))
            _ <- inputMolecule(List(bigInt2, bigInt3)).get.recover { case MoleculeException(err, _) =>
              err ==> "Can't apply multiple values to comparison function."
            }
          } yield ()
        ...
        "<=" - core { implicit conn =>
          ...
          for {
            _ <- oneData
            _ <- inputMolecule(Nil).get.map(_ ==> List(bigInt1, bigInt2, bigInt3))
            _ <- inputMolecule(List(bigInt2)).get.map(_ ==> List(bigInt1, bigInt2))
            _ <- inputMolecule(List(bigInt2, bigInt3)).get.recover { case MoleculeException(err, _) =>
              err ==> "Can't apply multiple values to comparison function."
            }
          } yield ()
      ...
      "Tacit" - {
        ...
        "Eq" - core { implicit conn =>
          val inputMolecule = m(Ns.str.a1.bigInt_(?))
          for {
            _ <- oneData
            ...
            _ <- inputMolecule(List(bigInt1, bigInt2)).get.map(_ ==> List(str1, str2))
          } yield ()
        }

        "not" - core { implicit conn =>
          val inputMolecule = m(Ns.str.a1.bigInt_.not(?))
          for {
            _ <- oneData
            _ <- inputMolecule(Nil).get.map(_ ==> List(str1, str2, str3))
            _ <- inputMolecule(List(bigInt1)).get.map(_ ==> List(str2, str3))
            _ <- inputMolecule(List(bigInt1, bigInt1)).get.map(_ ==> List(str2, str3))
            _ <- inputMolecule(List(bigInt1, bigInt2)).get.map(_ ==> List(str3))
          } yield ()
        }
        ...
        ">" - core { implicit conn =>
          ...
          for {
            _ <- oneData
            _ <- inputMolecule(Nil).get.map(_ ==> List(str1, str2, str3))
            _ <- inputMolecule(List(bigInt2)).get.map(_ ==> List(str3))
            _ <- inputMolecule(List(bigInt2, bigInt3)).get.recover { case MoleculeException(err, _) =>
              err ==> "Can't apply multiple values to comparison function."
            }
          } yield ()
        ...
        ">=" - core { implicit conn =>
          ...
          for {
            _ <- oneData
            _ <- inputMolecule(Nil).get.map(_ ==> List(str1, str2, str3))
            _ <- inputMolecule(List(bigInt2)).get.map(_ ==> List(str2, str3))
            _ <- inputMolecule(List(bigInt2, bigInt3)).get.recover { case MoleculeException(err, _) =>
              err ==> "Can't apply multiple values to comparison function."
            }
          } yield ()
        ...
        "<=" - core { implicit conn =>
          ...
          for {
            _ <- oneData
            _ <- inputMolecule(Nil).get.map(_ ==> List(str1, str2, str3))
            _ <- inputMolecule(List(bigInt2)).get.map(_ ==> List(str1, str2))
            _ <- inputMolecule(List(bigInt2, bigInt3)).get.recover { case MoleculeException(err, _) =>
              err ==> "Can't apply multiple values to comparison function."
            }
          } yield ()
    ...
    "Card many" - {
      ...
      "Mandatory" - {
        ...
        "Eq" - core { implicit conn =>
          val inputMolecule = m(Ns.bigInt.a1.bigInts(?))
          for {
            _ <- manyData
            _ <- inputMolecule(Nil).get.map(_ ==> Nil)
        ...
        "not" - core { implicit conn =>
          val inputMolecule = m(Ns.bigInt.a1.bigInts.not(?))
          val all           = List(
      ...
      "Tacit" - {
        ...
        "Eq" - core { implicit conn =>
          val inputMolecule = m(Ns.bigInt.a1.bigInts_(?))
          for {
            _ <- manyData
            ...
            _ <- inputMolecule(List(Set(bigInt2))).get.map(_ ==> List(bigInt1, bigInt2))
            _ <- inputMolecule(List(Set(bigInt3))).get.map(_ ==> List(bigInt2, bigInt3))
            ...
            _ <- inputMolecule(List(Set(bigInt4, bigInt5))).get.map(_ ==> List(bigInt4, bigInt5))
            ...
            _ <- inputMolecule(List(Set(bigInt1), Set(bigInt2))).get.map(_ ==> List(bigInt1, bigInt2))
            _ <- inputMolecule(List(Set(bigInt1), Set(bigInt3))).get.map(_ ==> List(bigInt1, bigInt2, bigInt3))

            _ <- inputMolecule(List(Set(bigInt1, bigInt2), Set(bigInt3))).get.map(_ ==> List(bigInt1, bigInt2, bigInt3))
            _ <- inputMolecule(List(Set(bigInt1), Set(bigInt2, bigInt3))).get.map(_ ==> List(bigInt1, bigInt2))
            _ <- inputMolecule(List(Set(bigInt1), Set(bigInt2), Set(bigInt3))).get.map(_ ==> List(bigInt1, bigInt2, bigInt3))

            _ <- inputMolecule(List(Set(bigInt1, bigInt2), Set(bigInt3, bigInt4))).get.map(_ ==> List(bigInt1, bigInt3))
          } yield ()
        }

        "not" - core { implicit conn =>
          val inputMolecule = m(Ns.bigInt.a1.bigInts_.not(?))
          for {
            _ <- Ns.bigInt.bigInts insert List(
              (bigInt1, Set(bigInt1, bigInt2, bigInt3)),
              (bigInt2, Set(bigInt2, bigInt3, bigInt4)),
            ...
            _ <- inputMolecule(Nil).get.map(_ ==> List(bigInt1, bigInt2, bigInt3))
            _ <- inputMolecule(Set[BigInt]()).get.map(_ ==> List(bigInt1, bigInt2, bigInt3))
            ...
            _ <- inputMolecule(Set(bigInt1)).get.map(_ ==> List(bigInt2, bigInt3))
            // Same as
            _ <- inputMolecule(List(Set(bigInt1))).get.map(_ ==> List(bigInt2, bigInt3))
            ...
            // Multiple values in a Set matches matches set-wise
            _ <- inputMolecule(Set(bigInt1, bigInt2)).get.map(_ ==> List(bigInt2, bigInt3))

            _ <- inputMolecule(Set(bigInt1), Set(bigInt3)).get.map(_ ==> Nil) // bigInt3 match all
            _ <- inputMolecule(Set(bigInt1, bigInt3)).get.map(_ ==> List(bigInt2, bigInt3))

            _ <- inputMolecule(Set(bigInt1), Set(bigInt2), Set(bigInt3)).get.map(_ ==> Nil) // bigInt3 match all
            _ <- inputMolecule(Set(bigInt1, bigInt2, bigInt3)).get.map(_ ==> List(bigInt2, bigInt3))

            _ <- inputMolecule(Set(bigInt1, bigInt2), Set(bigInt1)).get.map(_ ==> List(bigInt2, bigInt3))
        ...
        ">" - core { implicit conn =>
          ...
          for {
            _ <- manyData
            _ <- inputMolecule(Nil).get.map(_ ==> List(bigInt1, bigInt2, bigInt3, bigInt4, bigInt5))
            _ <- inputMolecule(List(Set[BigInt]())).get.map(_ ==> List(bigInt1, bigInt2, bigInt3, bigInt4, bigInt5))

            // (bigInt3, bigInt4), (bigInt4, bigInt5), (bigInt4, bigInt5, bigInt6)
            _ <- inputMolecule(List(Set(bigInt2))).get.map(_ ==> List(bigInt2, bigInt3, bigInt4, bigInt5))
        ...
        ">=" - core { implicit conn =>
          ...
          for {
            ...
            // (bigInt2, bigInt4), (bigInt3, bigInt4), (bigInt4, bigInt5), (bigInt4, bigInt5, bigInt6)
            _ <- inputMolecule(List(Set(bigInt2))).get.map(_ ==> List(bigInt1, bigInt2, bigInt3, bigInt4, bigInt5))
