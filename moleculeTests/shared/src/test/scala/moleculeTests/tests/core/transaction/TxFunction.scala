...
//  def inc(e: Long, amount: Int)(implicit conn: Future[Conn]): Seq[Statement] = {
...
//    _ <- Ns(e).int(newBalance).getUpdateStmts
//  }
//
//
//  // Constraint check before multiple updates
//  def transfer(from: Long, to: Long, amount: Int)(implicit conn: Future[Conn]): Seq[Statement] = {
...
//
//    // Update accounts
//    _ <- Ns(from).int(newFromBalance).getUpdateStmts ++ Ns(to).int(newToBalance).getUpdateStmts
//  }
//
//
//  // "Sub" tx fn - can be used on its own or in other tx functions
//  def withdraw(from: Long, amount: Int)(implicit conn: Future[Conn]): Seq[Statement] = {
//    val curFromBalance = Ns(from).int.get.headOption.getOrElse(0)
//    if (curFromBalance < amount)
//      throw new TxFnException(
//        s"Can't transfer $amount from account $from having a balance of only $curFromBalance.")
//
//    val newFromBalance = curFromBalance - amount
//    _ <- Ns(from).int(newFromBalance).getUpdateStmts
//  }
//
//
//  // "Sub" tx fn - can be used on its own or in other tx functions
//  def deposit(to: Long, amount: Int)(implicit conn: Future[Conn]): Seq[Statement] = {
//    val newToBalance = Ns(to).int.get.headOption.getOrElse(0) + amount
//    _ <- Ns(to).int(newToBalance).getUpdateStmts
//  }
//
//
//  // Compose tx function by calling other tx functions
//  def transferComposed(from: Long, to: Long, amount: Int)(implicit conn: Future[Conn]): Seq[Statement] = {
...
//  def addUser(username: String, age: Int)(implicit conn: Future[Conn]): Seq[Statement] = {
...
//
//    // Save valid User
//    _ <- Ns.str(username).int(age).getSaveStmts
//  }
//
//
//  // Constructor enforcing validation rules against db
//  def addUserPartiallyChecked(username: String, age: Int)(implicit conn: Future[Conn]): Seq[Statement] = {
//    // Checking that username hasn't been taken. Since this check runs inside
//    // the transaction we are guaranteed Atomicity. The requested
//    // username couldn't have been created between the check and saving.
//    val userCheck = Ns.str(username).get
...
//
//    "Synchronous / Asynchronous" - core { implicit conn =>
//
