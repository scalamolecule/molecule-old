...
import molecule.tests.core.expression.Aggregates._
import utest._
...
trait Base extends AsyncTestSuite {
  ...
  def oneData(implicit conn: Conn, ec: ExecutionContext): Future[Unit] = {
    for {
  ...
  def manyData(implicit conn: Conn, ec: ExecutionContext): Future[Unit] = {
    for {
