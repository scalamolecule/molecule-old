  ...
  lazy val tests = Tests {
    ...
    "Single cardinality-2 attribute - one entity" - core { implicit conn =>
      for {
        ...
        // Get one value (RuntimeException if no value)
        _ <- Ns.strs.get.map(_.head ==> strs1)
        _ <- Ns.ints.get.map(_.head ==> ints1)
        _ <- Ns.longs.get.map(_.head ==> longs1)
        _ <- Ns.doubles.get.map(_.head ==> doubles1)
    ...
    "Multiple cardinality-1 attributes - one entity" - core { implicit conn =>
      for {
        ...
        _ <- Ns.str.int.long.double.bool.date.uuid.uri.enum.get.map(_.head ==>
          (str1, int1, long1, double1, bool1, date1, uuid1, uri1, enum1))
      } yield ()
    }
    ...
    "Mixed cardinality attributes - multiple entities" - core { implicit conn =>
      for {
        _ <- Ns.str.ints.bool insert List(("foo", Set(1, 2), true), ("bar", Set(2, 3, 4), false))

        // (order not guaranteed)
        _ <- Ns.str.ints.bool.get === List(("bar", Set(2, 3, 4), false), ("foo", Set(1, 2), true))
      } yield ()
    }
  }
}