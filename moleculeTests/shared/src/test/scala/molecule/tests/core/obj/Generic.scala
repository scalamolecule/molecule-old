...
object Generic extends AsyncTestSuite with Helpers {
  ...
  lazy val tests = Tests {
    ...
    "Log" - core { implicit conn =>
      for {
        ...
        _ <- Log(Some(t), Some(t2)).e.a.v.t.tx.txInstant.op.getObjList.map { case List(txDatom, intDatom) =>
          // Transaction datom
          txDatom.e ==> tx
    ...
    "EAVT" - core { implicit conn =>
      for {
        (e, t, tx, txInstant, t2, tx2, txInstant2, t3, tx3, txInstant3) <- testData
        // Entity `e`
        _ <- EAVT(e).e.a.v.t.tx.txInstant.op.getObjList.map { case List(d1, d2) =>
          d1.e ==> e
          d1.a ==> ":Ns/str"
          d1.v ==> "a"
    ...
    "AEVT" - core { implicit conn =>
      for {
        ...
        // :Ns/int attribute
        _ <- AEVT(":Ns/int").e.a.v.t.tx.txInstant.op.getObjList.map { case List(d1, d2) =>
          d1.e ==> e
          d1.a ==> ":Ns/int"
          d1.v ==> 2
          d1.t ==> t2
