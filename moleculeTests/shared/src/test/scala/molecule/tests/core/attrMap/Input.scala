  ...
  lazy val tests = Tests {
    ...
    "Equality" - core { implicit conn =>
      for {
        ...
        _ <- inputMolecule.apply(Map("en" -> "Hello")).get === List(
          (3, Map("en" -> "Hello"))
        )

        // We can use a tacit attribute map (to only return the Int value):
        _ <- m(Ns.int.strMap_(?)).apply(Map("en" -> "Hello")).get === List(3)

        // Note that text searches for attribute maps are case-sensitive
        _ <- m(Ns.int.strMap_(?))(Map("en" -> "hello")).get === List()


        _ <- m(Ns.int.intMap(?))(Map("en" -> 10)).get === List(
          (1, Map("en" -> 10)),
          (2, Map("en" -> 10))
        )
        _ <- m(Ns.int.intMap_(?))(Map("en" -> 10)).get === List(1, 2)
      } yield ()
    }
    ...
    "Regex/partial search" - core { implicit conn =>
      for {
        ...
        _ <- m(Ns.int.strMap(?))(Map(".*" -> ".*He.*")).get === List(
          (1, Map("da" -> "Hejsa")),
          (3, Map("en" -> "Hello", "da" -> "Hej")),
          (4, Map("da" -> "Hej"))
        )
        _ <- m(Ns.int.strMap_(?))(Map(".*" -> ".*He.*")).get === List(1, 3, 4)

        // Retrieving a specific value for any key
        _ <- m(Ns.int.strMap(?))(Map(".*" -> "Hello")).get === List(
          (3, Map("en" -> "Hello"))
        )

        // Since other types than String cannot use regexes we can only
        // search for values equal to our needle:
        _ <- m(Ns.int.intMap(?))(Map(".*" -> 10)).get === List(
          (1, Map("en" -> 10)),
          (2, Map("en" -> 10, "da" -> 10))
        )
        _ <- m(Ns.int.intMap_(?))(Map(".*" -> 10)).get === List(1, 2)
      } yield ()
    }
    ...
    "Negation" - core { implicit conn =>
      for {
        ...
        _ <- m(Ns.int(2).strMap.!=(?))(Map(".*" -> "Bon giorno")).get === List(
        ...
        _ <- m(Ns.int(2).strMap.!=(?))(Map("fr" -> "Bonjour", "it" -> "Bon giorno")).get === List(
        ...
        _ <- m(Ns.int(2).strMap.!=(?))(Map("fr|it" -> "Bonjour|Bon giorno")).get === List(
          (2, Map("en" -> "Oh, Hi", "da" -> "Hilser"))
        )
        _ <- m(Ns.int(2).strMap_.!=(?))(Map("fr|it" -> "Bonjour|Bon giorno")).get === List(2)


        _ <- m(Ns.int.intMap.!=(?))(Map(".*" -> 30)).get === List(
          (1, Map("en" -> 10)),
          (2, Map("en" -> 10, "da" -> 10, "fr" -> 20))
        )
        _ <- m(Ns.int.intMap_.!=(?))(Map(".*" -> 30)).get === List(1, 2)
    ...
    "Comparison" - core { implicit conn =>
      for {
        ...
        _ <- m(Ns.int.strMap.>(?))(Map(".*" -> "Hej")).get === List(
        ...
        _ <- m(Ns.int.intMap.>(?))(Map(".*" -> 20)).get === List(
          (1, Map("da" -> 30)),
          (2, Map("it" -> 30)),
          (3, Map("en" -> 30, "da" -> 30)),
          (4, Map("da" -> 30))
        ...
        _ <- m(Ns.int.intMap_.>(?))(Map(".*" -> 20)).get === List(1, 2, 3, 4)
        _ <- m(Ns.int.intMap_.>=(?))(Map(".*" -> 20)).get === List(1, 2, 3, 4)
        _ <- m(Ns.int.intMap_.<=(?))(Map(".*" -> 20)).get === List(1, 2)
        _ <- m(Ns.int.intMap_.<(?))(Map(".*" -> 20)).get === List(1, 2)


        // Date
        _ <- m(Ns.int.dateMap.>(?))(Map(".*" -> date2)).get === List(
          (1, Map("da" -> date3)),
          (2, Map("it" -> date3)),
        ...
        _ <- m(Ns.int.dateMap_.>(?))(Map(".*" -> date2)).get === List(1, 2, 3, 4)
        _ <- m(Ns.int.dateMap_.>=(?))(Map(".*" -> date2)).get === List(1, 2, 3, 4)
        _ <- m(Ns.int.dateMap_.<=(?))(Map(".*" -> date2)).get === List(1, 2)
        _ <- m(Ns.int.dateMap_.<(?))(Map(".*" -> date2)).get === List(1, 2)
      } yield ()
    }
    ...
    "Different keys, multiple values" - core { implicit conn =>
      for {
        ...
        _ <- m(Ns.int.strMap(?))(Map("en" -> "Hello", "da" -> "Hej")).get === List(
          (3, Map("en" -> "Hello", "da" -> "Hej")),
          (4, Map("da" -> "Hej"))
        )
        // Or tie them together with a regex
        _ <- m(Ns.int.strMap(?))(Map(".*" -> "Hello|Hej")).get === List(
          (3, Map("en" -> "Hello", "da" -> "Hej")),
          (4, Map("da" -> "Hej"))
        )

        // Using `contains` is redundant since it translates to the same as above
        _ <- m(Ns.int.strMap.contains(?))(Map(".*" -> "Hello|Hej")).get === List(
          (3, Map("en" -> "Hello", "da" -> "Hej")),
          (4, Map("da" -> "Hej"))
        )
        _ <- m(Ns.int.strMap_.contains(?))(Map(".*" -> "Hello|Hej")).get === List(3, 4)

        // We can also mix varying pairs with regexes
        _ <- m(Ns.int.strMap(?))(Map("en" -> ".*Hi.*|.*He.*", "fr" -> "Bo.*")).get === List(
          (1, Map("en" -> "Hi there")),
          (2, Map("fr" -> "Bonjour", "en" -> "Oh, Hi")),
          (3, Map("en" -> "Hello"))
        )

        _ <- m(Ns.int.intMap(?))(Map("en" -> 10, "fr" -> 20)).get === List(
          (1, Map("en" -> 10)),
          (2, Map("fr" -> 20, "en" -> 10))
        )
        _ <- m(Ns.int.intMap_(?))(Map("en" -> 10, "fr" -> 20)).get === List(1, 2)
      } yield ()
    }
    ...
    "One key, multiple values" - core { implicit conn =>
      ...
      for {
        ...
        _ <- m(Ns.int.strMap(?))(Map("en" -> ".*Hi.*"), Map("en" -> ".*He.*")).get === hi_he

        // or a Seq of Maps (useful if saved in a variable)
        _ <- m(Ns.int.strMap(?))(Seq(Map("en" -> ".*Hi.*"), Map("en" -> ".*He.*"))).get === hi_he
        _ <- m(Ns.int.strMap(?))(maps).get === hi_he

        // The easiest solution is though to use a regEx
        _ <- m(Ns.int.strMap(?))(Map("en" -> ".*Hi.*|.*He.*")).get === hi_he


        // With other types we need to apply multiple Maps if we have multiple
        // identical keys with varying values to lookup
        _ <- m(Ns.int.intMap(?))(Map("en" -> 10), Map("en" -> 30)).get === List(
          (1, Map("en" -> 10)),
          (2, Map("en" -> 10)),
          (3, Map("en" -> 30))
        )
        ...
        // 3 pairs
        _ <- m(Ns.int.intMap(?))(Map("en" -> 10), Map("en" -> 30), Map("fr" -> 20)).get === List(
          (1, Map("en" -> 10)),
          (2, Map("fr" -> 20, "en" -> 10)),
          (3, Map("en" -> 30))
        )
