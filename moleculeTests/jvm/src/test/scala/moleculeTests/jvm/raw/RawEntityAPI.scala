...
// only for internal molecule use
...
object RawEntityAPI extends AsyncTestSuite {
  ...
  lazy val tests = Tests {
    ...
    "rawValue" - core { implicit conn =>
      for {
        e <- Ns.str("a").int(1).save.map(_.eid)

        _ <- e.rawValue(":Ns/str").map(_ ==> "a")
        _ <- e.rawValue(":Ns/int").map(_ ==> 1)
      } yield ()
    }
  }
}
