  ...
  "Async" in new CoreSetup {
    ...
    if (system == SystemPeer) {
      ...
      Ns.str.int insertAsync List(
        ...
        ("c", 3),
        ("d", 4)
      ) collect { case Right(tx) => // tx report from successful insert transaction
        // 4 inserted entities
        ...
        Ns(a, b).int(5).updateAsync.collect { case Right(tx2) => // tx report from successful update transaction
          // Current data
