package molecule.jvm.core.crud.insert
...
class InsertInput extends TestSpec {
  ...
  "Card one" in new CoreSetup {
    ...
    val insertAll = Ns.str.int.long.double.bool.date.uuid.uri.enum.insert

    // Var-arg for single entity
    insertAll(" ", 0, 0L, 0.0, false, date0, uuid0, uri0, "enum0")
  ...
  "Card many" in new CoreSetup {
    ...
    val insertAlls = Ns.strs.ints.longs.doubles.dates.uuids.uris.enums.insert

    insertAlls(Set(" "), Set(0), Set(0L), Set(0.0), Set(date0), Set(uuid0), Set(uri0), Set("enum0"))
    insertAlls(List(
