...
class UpdateMultipleAttributes extends TestSpec {
  ...
  "Async" in new CoreSetup {
    ...
    if (system == SystemPeer) {
      ...
      // Initial data
      Ns.int.str insertAsync List((1, "a"), (2, "b")) map { case Right(tx) => // tx report from successful insert transaction
        // 2 inserted entities
        val List(e1, e2) = tx.eids
        Ns.int.get === List((1, "a"), (2, "b"))

        // Update second entity asynchronously
        Ns(e2).int(42).str("hello world").updateAsync.map { case Right(tx2) => // tx report from successful update transaction
          // Current data
