    ...
    q"""
        ...
        val fields: Seq[String] = {
          ...
          val fields0 = $model.elements.foldLeft(Seq.empty[String], Seq.empty[String]) {
            ...
            case ((refs, fields), other)                            => throw new GetJsonException("Unexpected Molecule element: " + other)
          }._2

          if (fields0.length != $typesLength)
            throw new GetJsonException("Unexpected json field mismatch: model attr count doesn't match output types count." +
