...
import molecule.macros.exception.NestedTuplesException

/** Nested tuples helper methods. */
case class NestedTuples(modelE: Model, queryE: Query) {
  ...
  lazy val flatModel: Seq[Element] = {
    ...
    if (elements.size != queryE.f.outputs.size)
      throw new NestedTuplesException("Flattened model elements (" + elements.size + ") don't match query outputs (" + queryE.f.outputs.size + "):\n" +
        modelE + "\n----------------\n" + elements.mkString("\n") + "\n----------------\n" + queryE + "\n----------------\n")
    elements
  }
