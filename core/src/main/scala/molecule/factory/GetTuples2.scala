...
trait GetTuples2[Ctx <: Context] extends GetJson[Ctx] {
  import c.universe._
...
//         println(i + ": " + value.toString)
  ...
  def cast2(query: Tree, row: Tree, tpes: Seq[Type], zz: Tree, i: Tree): Tree = {
    val value: Tree =
      q"""
         val value = if($i >= $row.size) null else $row.get($i)
//         println(i + ": " + value.toString)
...
//    val tq"$name: $TypeName" = tq"$tpe0"
    val tpe: Type =  {
      x(1
        , c.typecheck(q"Seq(..$tpes).apply($zz)")
        , c.typecheck(q"Seq(..$tpes).apply($zz)").tpe
      )

//      c.typecheck(tpe0).tpe
      c.typecheck(q"Seq(..$tpes).apply($zz)").tpe
    }
    tpe match {
      case t if t <:< typeOf[Option[Map[String, _]]] => castOptionMap(value, t)
...
//    def nestedTuplesX(query: Tree, rows: Tree, tpes: Seq[Type]) = {
//      q"""
//          object hej {
//
//
//         def apply() = if ($rows.isEmpty) {
//              Seq[(..$tpes)]()
//            } else {
...
//  //println("---- " + indexMap)
//
//              ${recurseNested(query, rows, tpes, q"entityIndexes", q"manyRefIndexes", q"indexMap")}
//            }
//          }
//         hej()
//        """
//    }
//
//    def recurseNestedX(query: Tree, rows: Tree, tpes: Seq[Type], entityIndexes: Tree, manyRefIndexes: Tree, indexMap: Tree) = {
//      q"""
//          val sortedRows = ${sortRows(entityIndexes, q"$rows.toSeq")}
//
...
//
//            val tpl = ${resolveNested(query, tpes, q"prevTpl", q"prevRow", q"row", 0, 1, q"$entityIndexes.size - 1", 1, entityIndexes, manyRefIndexes, indexMap)}
//
//  //println("TPL1 " + tpl)
//
...
//    }
//
//    def resolveNestedX(query: Tree, tpes: Seq[Type], prevTpl: Tree, prevRow: Tree, row: Tree, entityIndex: Int,
//                      depth: Int, maxDepth: Tree, shift: Int, entityIndexes: Tree, manyRefIndexes: Tree, indexMap: Tree): Tree = {
...
//      val isNewManyRef = q"$manyRefIndexes.nonEmpty && $prevEnt != 0L && $prevRow.apply($manyRefIndexes.head) != $row.apply($manyRefIndexes.head)"
//      val values = getValues(query, tpes, prevTpl, prevRow, row, entityIndex, depth, maxDepth, shift, isNewNested, isNewManyRef, entityIndexes, manyRefIndexes, indexMap)
//      q"(..$values).asInstanceOf[(..$tpes)]"
//    }
//
//    def getValuesX(query: Tree, tpes: Seq[Type], prevTpl: Tree, prevRow: Tree, row: Tree, entityIndex: Int,
//                  depth: Int, maxDepth: Tree, shift: Int, isNewNested: Tree, isNewManyRef: Tree, entityIndexes: Tree, manyRefIndexes: Tree, indexMap: Tree) = {
//      tpes.zipWithIndex.foldLeft(shift, Seq.empty[Tree]) { case ((shift, vs), (t, typeIndex)) =>
//        t match {
//
...
//            (shift + 22, vs :+ resolve(query, tpes, nestedTpes, prevTpl, prevRow, row, typeIndex, entityIndex, depth, maxDepth, shift, isNewNested, isNewManyRef, entityIndexes, manyRefIndexes, indexMap))
//
...
//            (shift + 21, vs :+ resolve(query, tpes, nestedTpes, prevTpl, prevRow, row, typeIndex, entityIndex, depth, maxDepth, shift, isNewNested, isNewManyRef, entityIndexes, manyRefIndexes, indexMap))
//
...
//            (shift + 20, vs :+ resolve(query, tpes, nestedTpes, prevTpl, prevRow, row, typeIndex, entityIndex, depth, maxDepth, shift, isNewNested, isNewManyRef, entityIndexes, manyRefIndexes, indexMap))
//
...
//            (shift + 19, vs :+ resolve(query, tpes, nestedTpes, prevTpl, prevRow, row, typeIndex, entityIndex, depth, maxDepth, shift, isNewNested, isNewManyRef, entityIndexes, manyRefIndexes, indexMap))
//
...
//            (shift + 18, vs :+ resolve(query, tpes, nestedTpes, prevTpl, prevRow, row, typeIndex, entityIndex, depth, maxDepth, shift, isNewNested, isNewManyRef, entityIndexes, manyRefIndexes, indexMap))
//
...
//            (shift + 17, vs :+ resolve(query, tpes, nestedTpes, prevTpl, prevRow, row, typeIndex, entityIndex, depth, maxDepth, shift, isNewNested, isNewManyRef, entityIndexes, manyRefIndexes, indexMap))
//
...
//            (shift + 16, vs :+ resolve(query, tpes, nestedTpes, prevTpl, prevRow, row, typeIndex, entityIndex, depth, maxDepth, shift, isNewNested, isNewManyRef, entityIndexes, manyRefIndexes, indexMap))
//
...
//            (shift + 15, vs :+ resolve(query, tpes, nestedTpes, prevTpl, prevRow, row, typeIndex, entityIndex, depth, maxDepth, shift, isNewNested, isNewManyRef, entityIndexes, manyRefIndexes, indexMap))
//
...
//            (shift + 14, vs :+ resolve(query, tpes, nestedTpes, prevTpl, prevRow, row, typeIndex, entityIndex, depth, maxDepth, shift, isNewNested, isNewManyRef, entityIndexes, manyRefIndexes, indexMap))
//
...
//            (shift + 13, vs :+ resolve(query, tpes, nestedTpes, prevTpl, prevRow, row, typeIndex, entityIndex, depth, maxDepth, shift, isNewNested, isNewManyRef, entityIndexes, manyRefIndexes, indexMap))
//
...
//            (shift + 12, vs :+ resolve(query, tpes, nestedTpes, prevTpl, prevRow, row, typeIndex, entityIndex, depth, maxDepth, shift, isNewNested, isNewManyRef, entityIndexes, manyRefIndexes, indexMap))
//
...
//            (shift + 11, vs :+ resolve(query, tpes, nestedTpes, prevTpl, prevRow, row, typeIndex, entityIndex, depth, maxDepth, shift, isNewNested, isNewManyRef, entityIndexes, manyRefIndexes, indexMap))
//
...
//            (shift + 10, vs :+ resolve(query, tpes, nestedTpes, prevTpl, prevRow, row, typeIndex, entityIndex, depth, maxDepth, shift, isNewNested, isNewManyRef, entityIndexes, manyRefIndexes, indexMap))
//
...
//            (shift + 9, vs :+ resolve(query, tpes, nestedTpes, prevTpl, prevRow, row, typeIndex, entityIndex, depth, maxDepth, shift, isNewNested, isNewManyRef, entityIndexes, manyRefIndexes, indexMap))
//
...
//            (shift + 8, vs :+ resolve(query, tpes, nestedTpes, prevTpl, prevRow, row, typeIndex, entityIndex, depth, maxDepth, shift, isNewNested, isNewManyRef, entityIndexes, manyRefIndexes, indexMap))
//
...
//            (shift + 7, vs :+ resolve(query, tpes, nestedTpes, prevTpl, prevRow, row, typeIndex, entityIndex, depth, maxDepth, shift, isNewNested, isNewManyRef, entityIndexes, manyRefIndexes, indexMap))
//
...
//            (shift + 6, vs :+ resolve(query, tpes, nestedTpes, prevTpl, prevRow, row, typeIndex, entityIndex, depth, maxDepth, shift, isNewNested, isNewManyRef, entityIndexes, manyRefIndexes, indexMap))
//
...
//            (shift + 5, vs :+ resolve(query, tpes, nestedTpes, prevTpl, prevRow, row, typeIndex, entityIndex, depth, maxDepth, shift, isNewNested, isNewManyRef, entityIndexes, manyRefIndexes, indexMap))
//
...
//            (shift + 4, vs :+ resolve(query, tpes, nestedTpes, prevTpl, prevRow, row, typeIndex, entityIndex, depth, maxDepth, shift, isNewNested, isNewManyRef, entityIndexes, manyRefIndexes, indexMap))
//
...
//            (shift + 3, vs :+ resolve(query, tpes, nestedTpes, prevTpl, prevRow, row, typeIndex, entityIndex, depth, maxDepth, shift, isNewNested, isNewManyRef, entityIndexes, manyRefIndexes, indexMap))
//
...
//            (shift + 2, vs :+ resolve(query, tpes, nestedTpes, prevTpl, prevRow, row, typeIndex, entityIndex, depth, maxDepth, shift, isNewNested, isNewManyRef, entityIndexes, manyRefIndexes, indexMap))
//
//          case tpe if tpe <:< weakTypeOf[Seq[_]] =>
//            val nestedTpe = tpe.baseType(weakTypeOf[Seq[_]].typeSymbol).typeArgs.head
//            (shift + 1, vs :+ resolve(query, tpes, Seq(nestedTpe), prevTpl, prevRow, row, typeIndex, entityIndex, depth, maxDepth, shift, isNewNested, isNewManyRef, entityIndexes, manyRefIndexes, indexMap))
//
//          case tpe =>
...
//
//
//    def resolveX(query: Tree, tpes: Seq[Type], nestedTpes: Seq[Type], prevTpl: Tree, prevRow: Tree, row: Tree, typeIndex: Int,
//                entityIndex: Int, depth: Int, maxDepth: Tree, shift: Int, isNewNested: Tree, isNewManyRef: Tree, entityIndexes: Tree, manyRefIndexes: Tree, indexMap: Tree) = {
...
//          prevRow, row, rowIndex, depth + 1, maxDepth, shift, entityIndexes, manyRefIndexes, indexMap)
//      }
//          Seq(toAdd)
//
...
//      """
  ...
  //
  //          val added = if (true || $isNewNested) {
  ////          val added = if (isNewNested) {
  //            $prevTpl.get.asInstanceOf[Seq[(..$nestedTpes)]] :+ toAdd
  //          } else {
  //            $prevTpl.get.asInstanceOf[Seq[(..$nestedTpes)]].init :+ toAdd
  //          }
  //
  //          println(tab + "b added  : " + added + "    " + newNested)
  //          added
  //
  //
  //        } else {
...
//  def nestedTuples(query: Tree, rows: Tree, tpes: Seq[Type]) = {null}

  def getNestedTuples(tpes: Seq[Type]) =
    q"""
      def nestedTuplesXX(_modelE: Model, _queryE: Query, rows: Iterable[java.util.List[AnyRef]]) = {
...
//            val tpl = null.asInstanceOf[(..tpes)]
            val tpl = getResolvedNested(_queryE,
                                        Seq(..$tpes)
                                        prevTpl,
                                        prevRow,
                                        row,
...
//        null.asInstanceOf[Seq[(..tpes)]]
      }
"""

  def getNestedTuples2 =
    q"""
      def getResolvedNested(query: Query,
                            tpes: Seq[Type],
                            prevTpl: Option[Product],
                            prevRow: Seq[Any],
                            row: Seq[Any],
                            maxDepth: Int,
                            entityIndexes: Seq[Int],
                            manyRefIndexes: Seq[Int],
                            indexMap: Map[Int, Int]) = {

        val prevEnt = if(prevRow.head == 0L) 0L else prevRow.head.asInstanceOf[Long]
        val curEnt = row.head.asInstanceOf[Long]
...
//                               indexMap)
//
//        values.asInstanceOf[(..tpes)]
        null.asInstanceOf[tpes]
      }
    """

  def getNestedTuples3 =
    q"""
      def getValues(query: Query,
                    tpes: Seq[Type],
                    prevTpl: Option[Product],
                    prevRow: Seq[Any],
                    row: Seq[Any],
...
//              (shift + nestedTpes.length, vs :+ resolve(query, tpes, nestedTpes, prevTpl, prevRow, row, typeIndex, entityIndex, depth, maxDepth, shift, isNewNested, isNewManyRef, entityIndexes, manyRefIndexes, indexMap))
//
//            case tpe if tpe <:< weakTypeOf[Seq[_]] =>
//              val nestedTpe = tpe.baseType(weakTypeOf[Seq[_]].typeSymbol).typeArgs.head
//              (shift + 1, vs :+ resolve(query,
//                                        tpes,
//                                        Seq(nestedTpe),
//                                        prevTpl,
//                                        prevRow,
...
//
//            case tpe =>
//              (shift, vs :+ {cast2(q"query", q"row.asJava", tpes, q"typeIndex", q"indexMap(entityIndex + shift + typeIndex)")})
//          }
//        }._2
...
"""
  ...
  def getNestedTuples4 =
    q"""
      def resolve(query: Tree,
                  tpes: Seq[Type],
                  nestedTpes: Seq[Type],
                  prevTpl: Tree,
                  prevRow: Tree,
          ...
          if (prevTpl.isEmpty || isNewNested || isNewManyRef) {
            ...
            val toAdd = resolveNested(query,
                                      nestedTpes,
                                      Some(prevTpl.get.asInstanceOf[Seq[nestedTpes]].last.asInstanceOf[nestedTpes]),
                                      prevRow,
                                      row,
                                      rowIndex,
                                      depth + 1,
                                      ...
                                      entityIndexes,
                                      manyRefIndexes,
                                      indexMap).asInstanceOf[nestedTpes]
            ...
            val toAdd = resolveNested(query,
                                      nestedTpes,
                                      Some(prevTpl.get.asInstanceOf[tpes].productElement(typeIndex).asInstanceOf[Seq[nestedTpes]].last.asInstanceOf[nestedTpes]),
                                      prevRow,
                                      row,
                                      rowIndex,
                                      depth + 1,
...
//    val isNewManyRef = q"manyRefIndexes.nonEmpty && $prevEnt != 0L && $prevRow.apply(manyRefIndexes.head) != $row.apply(manyRefIndexes.head)"
//
//    lazy val values = tpes.zipWithIndex.foldLeft(shift, Seq.empty[Tree]) { case ((shift, vs), (t, typeIndex)) =>
//      t match {
//
//        case tpe if tpe <:< weakTypeOf[Seq[(_, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _)]] =>
//          val nestedTpes = tpe.baseType(weakTypeOf[Seq[_]].typeSymbol).typeArgs.head.baseType(weakTypeOf[(_, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _)].typeSymbol).typeArgs
//          (shift + 22, vs :+ resolve(nestedTpes, typeIndex))
//
//        case tpe if tpe <:< weakTypeOf[Seq[(_, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _)]] =>
//          val nestedTpes = tpe.baseType(weakTypeOf[Seq[_]].typeSymbol).typeArgs.head.baseType(weakTypeOf[(_, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _)].typeSymbol).typeArgs
//          (shift + 21, vs :+ resolve(nestedTpes, typeIndex))
//
//        case tpe if tpe <:< weakTypeOf[Seq[(_, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _)]] =>
//          val nestedTpes = tpe.baseType(weakTypeOf[Seq[_]].typeSymbol).typeArgs.head.baseType(weakTypeOf[(_, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _)].typeSymbol).typeArgs
//          (shift + 20, vs :+ resolve(nestedTpes, typeIndex))
//
//        case tpe if tpe <:< weakTypeOf[Seq[(_, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _)]] =>
//          val nestedTpes = tpe.baseType(weakTypeOf[Seq[_]].typeSymbol).typeArgs.head.baseType(weakTypeOf[(_, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _)].typeSymbol).typeArgs
//          (shift + 19, vs :+ resolve(nestedTpes, typeIndex))
//
//        case tpe if tpe <:< weakTypeOf[Seq[(_, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _)]] =>
//          val nestedTpes = tpe.baseType(weakTypeOf[Seq[_]].typeSymbol).typeArgs.head.baseType(weakTypeOf[(_, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _)].typeSymbol).typeArgs
//          (shift + 18, vs :+ resolve(nestedTpes, typeIndex))
//
//        case tpe if tpe <:< weakTypeOf[Seq[(_, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _)]] =>
//          val nestedTpes = tpe.baseType(weakTypeOf[Seq[_]].typeSymbol).typeArgs.head.baseType(weakTypeOf[(_, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _)].typeSymbol).typeArgs
//          (shift + 17, vs :+ resolve(nestedTpes, typeIndex))
//
//        case tpe if tpe <:< weakTypeOf[Seq[(_, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _)]] =>
//          val nestedTpes = tpe.baseType(weakTypeOf[Seq[_]].typeSymbol).typeArgs.head.baseType(weakTypeOf[(_, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _)].typeSymbol).typeArgs
//          (shift + 16, vs :+ resolve(nestedTpes, typeIndex))
//
//        case tpe if tpe <:< weakTypeOf[Seq[(_, _, _, _, _, _, _, _, _, _, _, _, _, _, _)]] =>
//          val nestedTpes = tpe.baseType(weakTypeOf[Seq[_]].typeSymbol).typeArgs.head.baseType(weakTypeOf[(_, _, _, _, _, _, _, _, _, _, _, _, _, _, _)].typeSymbol).typeArgs
//          (shift + 15, vs :+ resolve(nestedTpes, typeIndex))
//
//        case tpe if tpe <:< weakTypeOf[Seq[(_, _, _, _, _, _, _, _, _, _, _, _, _, _)]] =>
//          val nestedTpes = tpe.baseType(weakTypeOf[Seq[_]].typeSymbol).typeArgs.head.baseType(weakTypeOf[(_, _, _, _, _, _, _, _, _, _, _, _, _, _)].typeSymbol).typeArgs
//          (shift + 14, vs :+ resolve(nestedTpes, typeIndex))
//
//        case tpe if tpe <:< weakTypeOf[Seq[(_, _, _, _, _, _, _, _, _, _, _, _, _)]] =>
//          val nestedTpes = tpe.baseType(weakTypeOf[Seq[_]].typeSymbol).typeArgs.head.baseType(weakTypeOf[(_, _, _, _, _, _, _, _, _, _, _, _, _)].typeSymbol).typeArgs
//          (shift + 13, vs :+ resolve(nestedTpes, typeIndex))
//
//        case tpe if tpe <:< weakTypeOf[Seq[(_, _, _, _, _, _, _, _, _, _, _, _)]] =>
//          val nestedTpes = tpe.baseType(weakTypeOf[Seq[_]].typeSymbol).typeArgs.head.baseType(weakTypeOf[(_, _, _, _, _, _, _, _, _, _, _, _)].typeSymbol).typeArgs
//          (shift + 12, vs :+ resolve(nestedTpes, typeIndex))
//
//        case tpe if tpe <:< weakTypeOf[Seq[(_, _, _, _, _, _, _, _, _, _, _)]] =>
//          val nestedTpes = tpe.baseType(weakTypeOf[Seq[_]].typeSymbol).typeArgs.head.baseType(weakTypeOf[(_, _, _, _, _, _, _, _, _, _, _)].typeSymbol).typeArgs
//          (shift + 11, vs :+ resolve(nestedTpes, typeIndex))
//
//        case tpe if tpe <:< weakTypeOf[Seq[(_, _, _, _, _, _, _, _, _, _)]] =>
//          val nestedTpes = tpe.baseType(weakTypeOf[Seq[_]].typeSymbol).typeArgs.head.baseType(weakTypeOf[(_, _, _, _, _, _, _, _, _, _)].typeSymbol).typeArgs
//          (shift + 10, vs :+ resolve(nestedTpes, typeIndex))
//
//        case tpe if tpe <:< weakTypeOf[Seq[(_, _, _, _, _, _, _, _, _)]] =>
//          val nestedTpes = tpe.baseType(weakTypeOf[Seq[_]].typeSymbol).typeArgs.head.baseType(weakTypeOf[(_, _, _, _, _, _, _, _, _)].typeSymbol).typeArgs
//          (shift + 9, vs :+ resolve(nestedTpes, typeIndex))
//
//        case tpe if tpe <:< weakTypeOf[Seq[(_, _, _, _, _, _, _, _)]] =>
//          val nestedTpes = tpe.baseType(weakTypeOf[Seq[_]].typeSymbol).typeArgs.head.baseType(weakTypeOf[(_, _, _, _, _, _, _, _)].typeSymbol).typeArgs
//          (shift + 8, vs :+ resolve(nestedTpes, typeIndex))
//
//        case tpe if tpe <:< weakTypeOf[Seq[(_, _, _, _, _, _, _)]] =>
//          val nestedTpes = tpe.baseType(weakTypeOf[Seq[_]].typeSymbol).typeArgs.head.baseType(weakTypeOf[(_, _, _, _, _, _, _)].typeSymbol).typeArgs
//          (shift + 7, vs :+ resolve(nestedTpes, typeIndex))
//
//        case tpe if tpe <:< weakTypeOf[Seq[(_, _, _, _, _, _)]] =>
//          val nestedTpes = tpe.baseType(weakTypeOf[Seq[_]].typeSymbol).typeArgs.head.baseType(weakTypeOf[(_, _, _, _, _, _)].typeSymbol).typeArgs
//          (shift + 6, vs :+ resolve(nestedTpes, typeIndex))
//
//        case tpe if tpe <:< weakTypeOf[Seq[(_, _, _, _, _)]] =>
//          val nestedTpes = tpe.baseType(weakTypeOf[Seq[_]].typeSymbol).typeArgs.head.baseType(weakTypeOf[(_, _, _, _, _)].typeSymbol).typeArgs
//          (shift + 5, vs :+ resolve(nestedTpes, typeIndex))
//
//        case tpe if tpe <:< weakTypeOf[Seq[(_, _, _, _)]] =>
//          val nestedTpes = tpe.baseType(weakTypeOf[Seq[_]].typeSymbol).typeArgs.head.baseType(weakTypeOf[(_, _, _, _)].typeSymbol).typeArgs
//          (shift + 4, vs :+ resolve(nestedTpes, typeIndex))
//
//        case tpe if tpe <:< weakTypeOf[Seq[(_, _, _)]] =>
//          val nestedTpes = tpe.baseType(weakTypeOf[Seq[_]].typeSymbol).typeArgs.head.baseType(weakTypeOf[(_, _, _)].typeSymbol).typeArgs
//          (shift + 3, vs :+ resolve(nestedTpes, typeIndex))
//
//        case tpe if tpe <:< weakTypeOf[Seq[(_, _)]] =>
//          val nestedTpes = tpe.baseType(weakTypeOf[Seq[_]].typeSymbol).typeArgs.head.baseType(weakTypeOf[(_, _)].typeSymbol).typeArgs
//          (shift + 2, vs :+ resolve(nestedTpes, typeIndex))
//
//        case tpe if tpe <:< weakTypeOf[Seq[_]] =>
//          val nestedTpe = tpe.baseType(weakTypeOf[Seq[_]].typeSymbol).typeArgs.head
