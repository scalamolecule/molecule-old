...
trait GetTuples6[Ctx <: Context] extends GetJson[Ctx] {
  import c.universe._
...
//  def hejsa = q"$NestedResolver"

  trait NestedResolver[OuterTpl <: Product] { self: molecule.api.MoleculeBase =>
    import scala.collection.JavaConverters._


    def getNestedTuples(_modelE: Model, _queryE: Query, rows: Iterable[java.util.List[AnyRef]]): Iterable[OuterTpl] = {
      if (rows.isEmpty) {
        Seq.empty[OuterTpl]
      } else {
        ...
        def resolveNested[Tpl <: Product](prevTpl: Option[Tpl], prevRow: Seq[Any], row: Seq[Any], entityIndex: Int, depth: Int, shift: Int): Tpl = {
...
//          }
//
//          val values = typeOf[Tpl]. .zipWithIndex.foldLeft(shift, Seq.empty[Tree]) { case ((shift, vs), (t, typeIndex)) =>
//            t match {
//
//              case tpe if typeOf[Tpl] <:< weakTypeOf[Product] =>
...
//
//              case tpe =>
//                val adjustedIndex = indexMap(entityIndex + shift + typeIndex)
//                (shift, vs :+ cast3[(_query, row.asJava, tpe, q"$adjustedIndex"))
//            }
//          }._2
...
//          values.asInstanceOf[Tpl]
        ...
        val resolvedTuples = sortedRows.foldLeft((Seq.empty[OuterTpl], None: Option[OuterTpl], Seq(0L), Seq[Any](0L), 1)) { case ((accTpls0, prevTpl, prevEntities, prevRow, r), row0) =>
          ...
          // Recurse tuples
          //                    val tpl = resolveNested[Tpl](_query, prevTpl, prevRow, row, 0, 1, maxDepth, 1, entityIndexes, manyRefIndexes, indexMap)
  ...
  def cast3[T](query: Query, row: jList[Any], i: Int): T = {
    val value: Any = if(i >= row.size) null else row.get(i)
    weakTypeOf[T] match {
//      case t if t <:< typeOf[Option[Map[String, _]]] => castOptionMap(value, t)
//      case t if t <:< typeOf[Option[Set[_]]]         => castOptionSet(value, t)
//      case t if t <:< typeOf[Option[_]]              => castOption(value, t)
...
//      case t if t <:< typeOf[Stream[_]]              => q"$value.asInstanceOf[LazySeq].asScala.toStream.asInstanceOf[$t]"
//      case t if t <:< typeOf[Set[_]]                 => castSet(value, t)
      case t                                         => castType2[T](query, value, i)
    }
  }


  def castType2[T](query: Query, value: Any, i: Int) = weakTypeOf[T] match {
//    case t if t <:< typeOf[Long]       => q"(if($value.isInstanceOf[String]) $value.asInstanceOf[String].toLong else $value).asInstanceOf[$t]"
//    case t if t <:< typeOf[Double]     => q"(if($value.isInstanceOf[String]) $value.asInstanceOf[String].toDouble else $value).asInstanceOf[$t]"
//    case t if t <:< typeOf[Boolean]    => q"(if($value.isInstanceOf[String]) $value.asInstanceOf[String].toBoolean else $value).asInstanceOf[$t]"
//    case t if t <:< typeOf[BigInt]     => q"BigInt($value.toString).asInstanceOf[$t]"
...
//      """
  ...
  //  def getNestedResolver = {
  //    q"""
  //      trait NestedResolver[T] { self: molecule.api.MoleculeBase =>
  //
  //       def getNested
  //        if ($rows.isEmpty) {
  //          Seq[(..$tpes)]()
  ...
  //
  //
  //        private lazy val _modelE = r.modelE
  //        private lazy val _queryE = r.queryE
  //
  //        private lazy val flatModel = {
  //          def recurse(element: Element): Seq[Element] = element match {
  //            case n: Nested                                             => n.elements flatMap recurse
  //            case a@Atom(_, attr, _, _, _, _, _, _) if attr.last == '_' => Seq()
  //            case a: Atom                                               => Seq(a)
