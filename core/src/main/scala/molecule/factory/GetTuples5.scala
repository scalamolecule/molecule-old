...
trait GetTuples5[Ctx <: Context] extends GetJson[Ctx] {
  import c.universe._
...
//println("---- " + indexMap)
       ...
       (sortedRows, entityIndexes, manyRefIndexes, indexMap)
      """
  }

  def getNestedTuples(query: Tree, tpes: Seq[Type], sortedRows: Tree, entityIndexes: Tree, manyRefIndexes: Tree, indexMap: Tree) = {
    q"""
...
//println("ACC " + accTpls)
  ...
  def resolveNested(query: Tree, tpes: Seq[Type], prevTpl: Tree, prevRow: Tree, row: Tree, entityIndex: Int, depth: Int, maxDepth: Tree, shift: Int, entityIndexes: Tree, manyRefIndexes: Tree, indexMap: Tree): Tree = {
