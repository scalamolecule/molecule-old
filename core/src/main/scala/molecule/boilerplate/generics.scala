...
trait Generic2[Ns2[_,_], Ns3[_,_,_], In2[_,_,_], In3[_,_,_,_], A, B] {
  ...
  def e_         : Ns2[A, B] with OneLong   [Ns2[A, B], In2[B, A, B]] = ???
  def a_         : Ns2[A, B] with OneString [Ns2[A, B], In2[B, A, B]] = ???
  def v_         : Ns2[A, B] with OneAny    [Ns2[A, B], In2[B, A, B]] = ???
  def ns_        : Ns2[A, B] with OneString [Ns2[A, B], In2[B, A, B]] = ???
  def tx_        : Ns2[A, B] with OneLong   [Ns2[A, B], In2[B, A, B]] = ???
  def t_         : Ns2[A, B] with OneLong   [Ns2[A, B], In2[B, A, B]] = ???
  def txInstant_ : Ns2[A, B] with OneDate   [Ns2[A, B], In2[B, A, B]] = ???
  def op_        : Ns2[A, B] with OneBoolean[Ns2[A, B], In2[B, A, B]] = ???
}
