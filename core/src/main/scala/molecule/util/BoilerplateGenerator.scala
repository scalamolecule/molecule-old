...
object Generator {
  ...
  def generate(srcDir: String, domainDirs: Seq[String]) = {
    ...
    val files = domainDirs flatMap { domainDir =>
      ...
      definitionFiles flatMap { definitionFile =>
        ...
        // Write schema file
        val schemaFile: File = d.pkg.split('.').toList.foldLeft(new File(srcDir))((file, pkg) => file / pkg) / "schema" / s"${d.domain}Schema.scala"
        IO.write(schemaFile, schemaBody(d))

        // Write namespace files
        val namespaceFiles = d.nss.map { ns =>
          val nsFile: File = d.pkg.split('.').toList.foldLeft(new File(srcDir))((file, pkg) => file / pkg) / "dsl" / firstLow(d.domain) / s"${ns.ns}.scala"
...
object BoilerplateGenerator {
  def main(args: Array[String]) {
    args.toList match {
      case srcDir :: domainDirs => Generator.generate(srcDir, domainDirs)
      case _                    => println(
        """
          |Error mesage...
        """.stripMargin.trim)
