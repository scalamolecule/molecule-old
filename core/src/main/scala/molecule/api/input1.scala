...
trait InputMolecule_1[I1] extends InputMolecule {
  ...
  def bindValues1(in1: Seq[I1]) = {
    ...
    if (values.size > 1)
      _query.copy(i = In(Seq(InVar(CollectionBinding(vars.head), Seq(values))), _query.i.rules, _query.i.ds))
    else
      _query.copy(i = In(Seq(InVar(ScalarBinding(vars.head), Seq(Seq(values.head)))), _query.i.rules, _query.i.ds))
  }
