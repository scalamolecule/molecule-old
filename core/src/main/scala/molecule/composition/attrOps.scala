...
/** == ValueAttrOps ==
  ...
  *
  * {{{
  *   Community.name.get === List("hello", "world") // returns `String` values
  *
  *   Community.name("hello")       // apply value (equals)
  *   Community.name.!=("hello")
  *   Community.name.not("hello")
  *   Community.name.<("k")         //
  *   Community.name.<=("k")
  *   Community.name.>("k")
  *   Community.name.>=("k")
  *   Community.name.>=("k")
  *
  *   Community.name(distinct)      // negation
  *   Community.name(count)         // negation
  *   Community.name(countDistinct) // negation
  *   Community.name(countDistinct) // negation
  * }}}
  *
  */
trait ValueAttrOps[Ns, In, T, U] {
  ...
  /** Filter attribute values with OR expression
    * {{{ Ns.attr(<v1> or <v2>) }}}
    * Matches all entities with attr having value v1 or v2
...
trait OneOptional[Ns, T] {
  /** Apply optional value to attribute
    */
  def apply(some: Option[T]): Ns with Attr = ???
}

trait FulltextSearchOps[Ns, In] {
