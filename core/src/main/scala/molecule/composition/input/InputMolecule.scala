...
import molecule.action.MoleculeBase
import molecule.ast.model.{Or, TermValue}
import molecule.ast.query.{Placeholder, Query, Var}


private[molecule] trait InputMolecule extends MoleculeBase {
  ...
  protected def varsAndPrefixes(query: Query) = query.i.inputs.collect {
