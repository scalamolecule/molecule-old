...
case class Model2Transaction(conn: Connection, model: Model) extends Helpers {
  ...
  val stmtsModel: Seq[Statement] = {
    ...
    def aRef(gs: Seq[Generic], card: Int = 0) = {
  ...
  private def valueStmts(stmts: Seq[Statement], e: Any, a: String, arg: Any, prefix: Option[String] = None, meta: String = "", otherEdgeId0: Option[AnyRef] = None): Seq[Statement] = {
    ...
    val newStmts = if (meta.startsWith("biRef")) arg match {
      ...
      val edgeA = e
      val edgeB = otherEdgeId0 getOrElse sys.error("[Model2Transaction:valueStmts (edgeProp)]  Missing id of other edge.")
  ...
  def saveStmts(): Seq[Statement] = {
    ...
    stmtsModel.foldLeft(None: Option[AnyRef], Seq[Statement]()) { case ((otherEdgeId, stmts), stmt) =>
      stmt match {
        ...
        case Add('e, a, refNs: String, meta) if meta.startsWith("edgeRef")   => val (edgeA, edgeB) = (tempId(refNs), Some(tempId(a))); (edgeB, valueStmts(stmts, lastE(stmts, a), a, edgeA, None, meta, edgeB))
        case Add('e, a, refNs: String, meta) if meta.startsWith("targetRef") => (None, valueStmts(stmts, lastE(stmts, a), a, tempId(refNs), None, meta, otherEdgeId))
        ...
        case Add('ns, a, _, _)                                               => (otherEdgeId, stmts)
