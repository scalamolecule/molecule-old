package molecule.coretests.meta
...
/** Datom meta attribute interface
  *
  * - `e` Entity id (Long)
...
class Datom extends CoreSpec {
  ...
  "Entities" >> {
    ...
    "1 entity" >> {
      ...
      // Several meta attributes can be asserted with a common entity id
      // Molecules with an applied entity id and only meta attributes
      ...
      // Meta attributes can be added in any order
  ...
  "Attributes" >> {
    ...
    "1 Attribute" >> {
      ...
      // Meta attributes after attribute with applied value
      ...
      // Meta attributes after attribute with applied operation
      ...
      // Meta attributes after attribute with applied aggregate keyword
    ...
    "Meta attributes before custom attribute" >> {

      // Generic entity id attribute before first attribute ok
      Ns.e.int.t.get === List((e1, 3, t3), (e2, 5, t5))

      // Other meta attributes not allowed before first attribute
      expectCompileError(
        "Ns.t.op.int.get",
        "molecule.transform.exception.Dsl2ModelException: " +
          "Can't add first attribute `int` after meta attributes (except `e` which is ok to have first). " +
          "Please add meta attributes `t`, `op` after `int`.")

      // Add meta attributes after custom attribute instead
    ...
    "Full scan" >> {
      ...
      // Meta attributes without an entity id applied or custom attributes added would
      // cause a full scan of the whole database and is not allowed
      expectCompileError(
        "Ns.e.a.v.t.get",
        "molecule.transform.exception.Dsl2ModelException: " +
          "Molecule with only meta attributes and no entity id(s) applied are not allowed since " +
  ...
  "Expressions, mandatory" >> {
    ...
    expectCompileError(
      """m(Ns.v.>(3))""",
      "molecule.transform.exception.Dsl2ModelException: " +
        "Can't compare meta values being of different types. Found: v.>(3)")
    ...
    // Meta attributes only allowed to aggregate `count`
    expectCompileError(
      """m(Ns.int.t(max))""",
      "molecule.transform.exception.Dsl2ModelException: " +
        "Meta attributes only allowed to aggregate `count`. Found: `max`")
    ok
  }
  ...
  "Expressions, tacit" >> {
    ...
    expectCompileError(
      """m(Ns.int.v_.<=(3))""",
      "molecule.transform.exception.Dsl2ModelException: " +
        "Can't compare meta values being of different types. Found: v_.<=(3)")
  ...
  "Multiple tx attributes" >> {
    ...
    // Tacit attributes can be followed by meta attributes
  ...
  "Optional tx data not allowed" >> {
    expectCompileError(
      """m(Ns.int$.tx.str)""",
      "molecule.transform.exception.Dsl2ModelException: Optional attributes (`int$`) can't be followed by meta transaction attributes (`tx`).")

    expectCompileError(
      """m(Ns.int$.t.str)""",
      "molecule.transform.exception.Dsl2ModelException: Optional attributes (`int$`) can't be followed by meta transaction attributes (`t`).")

    expectCompileError(
      """m(Ns.int$.txInstant.str)""",
      "molecule.transform.exception.Dsl2ModelException: Optional attributes (`int$`) can't be followed by meta transaction attributes (`txInstant`).")

    expectCompileError(
      """m(Ns.int$.op.str)""",
      "molecule.transform.exception.Dsl2ModelException: Optional attributes (`int$`) can't be followed by meta transaction attributes (`op`).")
