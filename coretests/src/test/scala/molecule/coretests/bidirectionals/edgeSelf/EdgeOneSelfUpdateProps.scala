  ...
  "Card-one" >> {
    ...
    "value" in new setup {
      ...
      (Person(ann).Loves.howWeMet("inSchool").update must throwA[IllegalArgumentException])
        ...
        s"[molecule.api.CheckModel.update_edgeComplete]  Can't update edge `Loves` " +
      ...
      Person.name("Ann" or "Ben").Loves.weight.get.toSeq.sortBy(_._1) === List(
        ("Ann", 7),
        ("Ben", 7)
      )
    ...
    "enum" in new setup {
      ...
      Person.name("Ann" or "Ben").Loves.howWeMet.get.toSeq.sortBy(_._1) === List(
        ("Ann", "atWork"),
        ("Ben", "atWork")
      )
    ...
    "ref" in new setup {
      ...
      // Current value
      Person.name("Ann" or "Ben").Loves.CoreQuality.name.get.toSeq.sortBy(_._1) === List(
        ("Ann", "Love"),
        ("Ben", "Love")
      )
