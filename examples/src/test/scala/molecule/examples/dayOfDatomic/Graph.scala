...
class Graph extends Specification with DatomicFacade {
  ...
  "Advanced hyperedge" >> {
    ...
    User.name.RoleInGroup(RoleInGroup.name.group.roles).insert.apply(List(
      ("User1", List(
    ...
    //
    //    User.name.apply(?).name.apply(?).RoleInGroup.apply(Role.name).Group.name(count) apply ("User1" and "User2")
    //
    //
    //    User.name("User1").RoleInGroup.apply(Role.name). Group.name(count).debug
