...
class Values extends Base {
  ...
  "OR semantics" in new Setup {
    ...
    // Entities with this or that value
    Ns.int.strMap("Hello", "Hej").get === List(
      (3, Map("en" -> "Hello")),
      (4, Map("da" -> "Hej"))
    )
    // We can't use 'or' notation since that is reserved for key/value pairs
    // Ns.int.strMap("Hello" or "Hej")

    Ns.int.strs(Set("Hello", "Hej")).get === List()
    ...
    // For the String type we can also use a regex
    Ns.int.strMap("He|Bo").get === List(
