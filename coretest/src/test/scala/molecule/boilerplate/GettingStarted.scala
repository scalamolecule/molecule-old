...
import molecule.boilerplate.dsl.modernGraph._
import molecule.boilerplate.schema._
...
class ModernGraphSetup(schema: Transaction) extends Scope with DatomicFacade {
  ...
  Knows.person.weight debugInsert Seq(
    (vadas, 0.5),
    (josh, 1.0)
  )
//  println(Person(marko).Knows.person.weight._model)

//  Person(marko).Knows.person.weight.debugInsert(Seq(
//    (vadas, 0.5),
//    (josh, 1.0)
//  ))

//  Person.name.Knows.weight.Person.name.debugInsert(Seq(
//      ("Marc", 0.5, "Johan"),
//      ("Marc", 0.8, "Jon")
//    ))
