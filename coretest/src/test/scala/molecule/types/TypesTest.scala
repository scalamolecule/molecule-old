...
class TypesTest extends CoreSpec {
  ...
  // Load data
  One.str.int.long.float.double.bool.date.uuid.uri.enum.Person.name insert (
    str1, int1, long1, float1, double1, bool1, date1, uuid1, uri1, enum1, name1)

  Many.strM.intM.longM.floatM.doubleM.dateM.uuidM.uriM.enumM.Persons.name insert (
    Set(str1, str2),
    Set(int1, int2),
    Set(long1, long2),
    Set(float1, float2),
  ...
  "Infer return types of cardinality-one attributes" >> {
    ...
    // Check return types form query results
    ...
    typed[String](One.Person.name.get.head)
  ...
  "Infer return types of cardinality-many attributes" >> {
    ...
    typed[String](Many.Persons.name.get.head)
