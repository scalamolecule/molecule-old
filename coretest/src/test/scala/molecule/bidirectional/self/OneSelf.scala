...
class OneSelf extends MoleculeSpec {
  ...
  "Save new" in new Setup {
    ...
    Person(adam).Spouse.name.one === "Lisa"
    Person(lisa).Spouse.name.one === "Adam"

    // Forth and back should bring us to the initial value (given Adam cardinality one ref)
    Person(adam).Spouse.name_.Spouse.name.one === "Adam"
    Person(lisa).Spouse.name_.Spouse.name.one === "Lisa"
  }
