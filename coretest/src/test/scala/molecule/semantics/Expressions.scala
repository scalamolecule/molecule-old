...
//    Ns.uri(uri2).get === List(uri2)
  ...
  "Apply multiple values (OR-semantics) to cardinality-one attribute" in new BaseSetup {
    ...
    // 3. List of values
//    val needles = List("c", "a")
//    Ns.str(needles).debug
//    Ns.str(needles).get.sorted === List("a")
...
//    Ns.uri(uri2).get === List(uri2)
  ...
  class ManyData extends CoreSetup {
...
//    Ns.enum insert List("enum1", "enum2")
  ...
  "Apply value(s) to cardinality-many attributes" in new ManyData {
