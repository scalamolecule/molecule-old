 ...
 * {{{
 * for {
 *   _ <- Schema.e(count).get.map(_.head ==> 27)                  // count of all attributes in schema
 *   _ <- Schema.a(countDistinct).get.map(_.head ==> 23)          // count of all distinct attribute names in schema
 *   _ <- Schema.ns("myNamepace").a(count).get.map(_.head ==> 3)  // count of attributes in MyNamespace
 * } yield ()
 * }}}
 *
