...
trait Packed2tplFlat extends Unpackers { self: BuildTplComposite =>
  ...
  def packed2tplFlat(obj: Obj, txMetas: Int): Tree = {
    ...
    def flatTxComposite: Tree = {
      ...
      xx(2, obj, txMetas, unpackers)
      q"(..$unpackers)"
    }

    if (txMetas <= 1) flat else flatTxComposite
  }
}
