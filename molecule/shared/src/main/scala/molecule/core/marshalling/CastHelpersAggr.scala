...
trait CastHelpersAggr extends CastHelpersOptNested {
  ...
  protected def castAggrOneListInt(row: jList[_], i: Int): List[Int] = {
    val it   = row.get(i).asInstanceOf[jList[_]].iterator
  ...
  protected def castAggrSingleSampleInt(row: jList[_], i: Int): Int =
    row.get(i).asInstanceOf[jList[_]].iterator.next.asInstanceOf[jLong].toInt

  protected def castAggrSingleSampleFloat(row: jList[_], i: Int): Float =
    row.get(i).asInstanceOf[jList[_]].iterator.next.asInstanceOf[jDouble].toFloat

  protected def castAggrSingleSampleURI(row: jList[_], i: Int): URI =
    row.get(i).asInstanceOf[jList[_]].iterator.next match {
      case uri: URI => uri
      case uriImpl  => new URI(uriImpl.toString)
    }

  protected def castAggrSingleSampleBigInt(row: jList[_], i: Int): BigInt =
    BigInt(row.get(i).asInstanceOf[jList[_]].iterator.next.toString)

  protected def castAggrSingleSampleBigDecimal(row: jList[_], i: Int): BigDecimal =
    BigDecimal(row.get(i).asInstanceOf[jList[_]].iterator.next.asInstanceOf[java.math.BigDecimal].toString)

  protected def castAggrSingleSample[T](row: jList[_], i: Int): T =
    row.get(i).asInstanceOf[jList[_]].iterator.next.asInstanceOf[T]
