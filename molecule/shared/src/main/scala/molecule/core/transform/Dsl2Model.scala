...
//  with BuildJsonComposite
  ...
  private[molecule] final def getModel(dsl: Tree): (
    ...
    def addLambdas(cast: richTree => Int => Tree, json: richTree => (Int, Int) => Tree, t: richTree): Unit = {
      if (t.name.last != '_') {
        if (post) {
    ...
    def resolveComposite(prev: Tree, p: richTree, subCompositeTree: Tree, sameNs: Boolean = false): Seq[Element] = {
      ...
      jsonss = List.empty[(Int, Int) => Tree] :: jsonss
    ...
    def resolveOptionalAttr(tree: Tree, t: richTree, prev: Tree, p: richTree, attrStr: String): Seq[Element] = {
      if (genericType == "schema") {
        ...
        addLambdas(
          if (optNestedLevel > 0) castOptNestedOptEnum else castOptEnum,
          if (optNestedLevel > 0) jsonOptNestedOptEnum else jsonOptEnum,
          t
        )
