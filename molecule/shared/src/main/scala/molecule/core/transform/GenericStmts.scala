...
abstract class GenericStmts(conn: Conn, model: Model) extends Helpers {
  ...
  val genericStmts: Seq[Statement] = {
    ...
    def resolveTx(elements: Seq[Element]): Seq[Statement] = elements.foldLeft("tx": Any, Seq[Statement]()) {
      case (_, Atom(nsFull, name, _, _, VarValue, _, _, _, _)) => err("stmtsModel",
    ...
    def resolveElement(eSlot: Any, stmts: Seq[Statement], element: Element): (Any, Seq[Statement]) = {
      (eSlot, element) match {
        // None
        case (eids@Eids(ids), Atom(nsFull, name, _, c, Fn("not", _), _, gs, _, _)) => (eids, stmts ++ ids.map(Retract(_, s":$nsFull/$name", Values(Eq(Nil)), bi(gs, c))))
        case (eid@Eid(id), Atom(nsFull, name, _, c, Fn("not", _), _, gs, _, _))    => (eid, stmts :+ Add(id, s":$nsFull/$name", Values(Eq(Nil)), bi(gs, c)))
        case (e, Atom(_, _, _, _, Fn("not", _), _, _, _, _))                       => (e, stmts)
        case (e, Atom(_, _, _, _, Eq(Seq(None)), _, _, _, _))                      => (e, stmts)

        // First
        case ("_", Generic(_, "e" | "e_", _, EntValue, _))            => ("__arg", stmts)
        case ("_", Generic(_, "e" | "e_", _, Eq(Seq(id: Long)), _))   => (Eid(id), stmts)
        case ("_", Generic(_, "e" | "e_", _, Eq(ids: Seq[_]), _))     => (Eids(ids), stmts)
        case ("_", Atom(nsFull, name, _, c, VarValue, _, gs, _, _))   => ("e", stmts :+ Add("__tempId", s":$nsFull/$name", "__arg", bi(gs, c)))
        case ("_", Atom(nsFull, name, _, c, value, prefix, gs, _, _)) => ("e", stmts :+ Add("__tempId", s":$nsFull/$name", Values(value, prefix), bi(gs, c)))
        ...
        case ("ec", Generic(_, "e" | "e_", _, EntValue, _))          => ("__arg", stmts)
        case ("ec", Generic(_, "e" | "e_", _, Eq(Seq(id: Long)), _)) => (Eid(id), stmts)
        case ("ec", Generic(_, "e" | "e_", _, Eq(ids: Seq[_]), _))   => (Eids(ids), stmts)

        // Entity ids applied to initial namespace
        case (eids@Eids(ids), Atom(nsFull, name, _, c, value@RetractValue(_), prefix, gs, _, _)) => (eids, stmts ++ ids.map(Retract(_, s":$nsFull/$name", Values(value, prefix), bi(gs, c))))
        case (eids@Eids(ids), Atom(nsFull, name, _, c, value, prefix, gs, _, _))                 => (eids, stmts ++ ids.map(Add(_, s":$nsFull/$name", Values(value, prefix), bi(gs, c))))
        ...
        case (eid@Eid(id), Atom(nsFull, name, _, c, value@RetractValue(_), prefix, gs, _, _)) => (eid, stmts :+ Retract(id, s":$nsFull/$name", Values(value, prefix), bi(gs, c)))
        case (eid@Eid(id), Atom(nsFull, name, _, c, value, prefix, gs, _, _))                 => (eid, stmts :+ Add(id, s":$nsFull/$name", Values(value, prefix), bi(gs, c)))
        case (Eid(id), Bond(nsFull, refAttr, _, c, gs))                                       => ("v", stmts :+ Add(id, s":$nsFull/$refAttr", "__tempId", bi(gs, c)))


        // Same namespace
        case ("e", Atom(nsFull, name, _, c, value@RetractValue(_), prefix, gs, _, _))   => ("e", stmts :+ Retract("e", s":$nsFull/$name", Values(value, prefix), bi(gs, c)))
        case ("e", Atom(nsFull, name, _, c, VarValue, _, gs, _, _)) if name.last == '$' => ("e", stmts :+ Add("e", s":$nsFull/${name.init}", "__arg", bi(gs, c)))
        case ("e", Atom(nsFull, name, _, c, VarValue, _, gs, _, _))                     => ("e", stmts :+ Add("e", s":$nsFull/$name", "__arg", bi(gs, c)))
        case ("e", Atom(nsFull, name, _, c, value, prefix, gs, _, _))                   => ("e", stmts :+ Add("e", s":$nsFull/$name", Values(value, prefix), bi(gs, c)))
        case ("e", Bond(nsFull, refAttr, refNs, c, gs))                                 => ("v", stmts :+ Add("e", s":$nsFull/$refAttr", s":$refNs", bi(gs, c)))

        // Same namespace - composite
        case ("ec", Atom(nsFull, name, _, c, value@RetractValue(_), prefix, gs, _, _))   => ("e", stmts :+ Retract("ec", s":$nsFull/$name", Values(value, prefix), bi(gs, c)))
        case ("ec", Atom(nsFull, name, _, c, VarValue, _, gs, _, _)) if name.last == '$' => ("e", stmts :+ Add("ec", s":$nsFull/${name.init}", "__arg", bi(gs, c)))
        case ("ec", Atom(nsFull, name, _, c, VarValue, _, gs, _, _))                     => ("e", stmts :+ Add("ec", s":$nsFull/$name", "__arg", bi(gs, c)))
        case ("ec", Atom(nsFull, name, _, c, value, prefix, gs, _, _))                   => ("e", stmts :+ Add("ec", s":$nsFull/$name", Values(value, prefix), bi(gs, c)))
        ...
        case ("tx", Atom(nsFull, name, _, c, value, prefix, _, _, _)) if name.last == '_' || name.last == '$'    => ("tx", stmts :+ Add("tx", s":$nsFull/${name.init}", Values(value, prefix), Card(c)))
        case ("tx", Atom(nsFull, name, _, c, value, prefix, _, _, _))                                            => ("tx", stmts :+ Add("tx", s":$nsFull/$name", Values(value, prefix), Card(c)))
        ...
        case ("txRef", Atom(nsFull, name, _, c, value, prefix, _, _, _)) if name.last == '_' || name.last == '$' => ("txRef", stmts :+ Add("txRef", s":$nsFull/${name.init}", Values(value, prefix), Card(c)))
        case ("txRef", Atom(nsFull, name, _, c, value, prefix, _, _, _))                                         => ("txRef", stmts :+ Add("txRef", s":$nsFull/$name", Values(value, prefix), Card(c)))
        case ("tx" | "txRef", other)                                                                             =>
          err("stmtsModel", s"Transaction data can only have references and attributes with applied value:\nMODEL: $model \nFOUND: $other\nSTMTS: $stmts")

        // Next namespace
        case ("v", Atom(nsFull, name, _, c, VarValue, _, gs, _, _))   => ("e", stmts :+ Add("v", s":$nsFull/$name", "__arg", bi(gs, c)))
        case ("v", Atom(nsFull, name, _, c, value, prefix, gs, _, _)) => ("e", stmts :+ Add("v", s":$nsFull/$name", Values(value, prefix), bi(gs, c)))
        ...
        case ("__arg", Atom(nsFull, name, _, c, VarValue, _, _, _, _))   => ("e", stmts :+ Add("remove_me", s":$nsFull/$name", "__arg", Card(c)) :+ Add("v", s":$nsFull/$name", "__arg", Card(c)))
        case ("__arg", Atom(nsFull, name, _, c, value, prefix, _, _, _)) => ("e", stmts :+ Add("remove_me", s":$nsFull/$name", "__arg", Card(c)) :+ Add("v", s":$nsFull/$name", Values(value, prefix), Card(c)))
    ...
    def replace$(elements: Seq[Element]): Seq[Element] = elements map {
      case a@Atom(_, attr, _, _, _, Some(enumPrefix), _, _, _)
        if attr.last == '$' && enumPrefix.init.last == '$'          => a.copy(attr = attr.init, enumPrefix = Some(enumPrefix.init.init + "/"))
      case a@Atom(_, attr, _, _, _, _, _, _, _) if attr.last == '$' => a.copy(attr = attr.init)
