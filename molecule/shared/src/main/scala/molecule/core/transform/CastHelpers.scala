...
//  protected def row2json(sb: StringBuilder, row: jList[AnyRef]): StringBuilder = ???
  ...
  protected def castOptOneApplyBigInt(row: jList[_], i: Int): Option[BigInt] = if (row.get(i) == null) {
    ...
    Some(BigInt(row.get(i).toString))
  }
  ...
  protected def castOptManyInt(row: jList[_], i: Int): Option[Set[Int]] = if (row.get(i) == null) {
    ...
    val it  = row.get(i).asInstanceOf[jMap[String, jList[_]]].values.iterator.next.iterator
    var set = Set.empty[Int]
    while (it.hasNext)
      set += it.next.asInstanceOf[jLong].toInt
    Some(set)
  ...
  protected def castOptOneRefAttr(row: jList[_], i: Int): Option[Long] = if (row.get(i) == null) {
    ...
    row.get(i).asInstanceOf[jMap[String, jList[_]]].values.iterator.next.asInstanceOf[jMap[_, _]].forEach {
      case _ if done                        =>
  ...
  protected def castOptOneEnum(row: jList[_], i: Int): Option[String] = if (row.get(i) == null) {
    ...
    Some(
      getKwName(
        row.get(i).asInstanceOf[jMap[String, AnyRef]].values.iterator.next
