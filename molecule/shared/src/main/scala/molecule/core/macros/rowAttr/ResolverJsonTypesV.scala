...
private[molecule] trait ResolverJsonTypesV extends TreeOps {
  val c: blackbox.Context

  import c.universe._

  def getResolverJson_Types(group: String, baseTpe: String, field: String, v: Tree, tabs: Int): Tree = group match {
    case "One"                  => json_OneAttr(baseTpe, field, v)
...
//    case "AggrManySingle"       => jsonAggrManySingle(baseTpe, field)
  ...
  def json_OneAttr(baseTpe: String, field: String, v: Tree): Tree = baseTpe match {
    case "String"     => q"json_OneQuoted(sb, $field, $v)"
    case "Int"        => q"json_One(sb, $field, $v)"
    case "Long"       => q"json_OneToString(sb, $field, $v)"
    case "Double"     => q"json_OneToString(sb, $field, $v)"
    case "Boolean"    => q"json_OneToString(sb, $field, $v)"
    case "Date"       => q"json_OneDate(sb, $field, $v)"
    case "UUID"       => q"json_OneQuoted(sb, $field, $v)"
    case "URI"        => q"json_OneQuoted(sb, $field, $v)"
    case "BigInt"     => q"json_OneToString(sb, $field, $v)"
    case "BigDecimal" => q"json_OneToString(sb, $field, $v)"
    case "Any"        => q"json_OneAny(sb, $field, $v)"
    case "enum"       => q"json_OneQuoted(sb, $field, $v)"
    case "ref"        => q"json_One(sb, $field, $v)"
  }
