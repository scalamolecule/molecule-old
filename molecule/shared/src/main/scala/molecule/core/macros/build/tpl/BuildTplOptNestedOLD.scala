...
private[molecule] trait BuildTplOptNestedOLD extends LambdaCastOptNested {
    ...
    val lambdas   = levelCasts.zipWithIndex.foldLeft(List.empty[Tree]) {
      ...
      case (acc, (casts, level)) => {
        ...
        val iterators = if (!optNestedRefIndexes.keySet.contains(level)) {
          q"val it0 = sub.next.asInstanceOf[jMap[AnyRef, AnyRef]].values().iterator()"
        } else {
          val extraIterators = optNestedRefIndexes(level).map { refIndex =>
            val itX = TermName("it" + (level * 100 + refIndex))
            q"""
               lazy val $itX = {
                 val valueMap = it0.next.asInstanceOf[jMap[AnyRef, AnyRef]]
          ...
          q"""
            val vs0 = sub.next.asInstanceOf[jMap[AnyRef, AnyRef]]
        ...
        val (none, subIterator) = if (level == 1)
          (
            q"null",
            q"""
              v.asInstanceOf[jMap[AnyRef, AnyRef]].values().iterator().next
               .asInstanceOf[jList[AnyRef]].iterator()
             """
          )
        else
          (q""""__none__"""",
            q"v.asInstanceOf[jList[AnyRef]].iterator()")
