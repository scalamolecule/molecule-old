...
trait NestedTuples[Obj, OuterTpl] extends jComparator[jList[AnyRef]] { self: Molecule_0[Obj, OuterTpl] =>
  ...
  final override def getIterable(implicit conn: Conn): Future[Iterable[OuterTpl]] = get(conn)
  ...
  // Generated by macro
  def outerTpl2obj(outerTpl: OuterTpl): Obj = ???
  ...
  /**
    ...
    * @return Iterable[Obj]
    */
  final override def getObjIterable(implicit conn: Conn): Future[Iterable[Obj]] = {
    getIterable.map(iterable =>
    new Iterable[Obj] {
      val tplIterable: Iterable[OuterTpl] = iterable
      ...
      override def iterator: Iterator[Obj] = new Iterator[Obj] {
        ...
        override def next(): Obj = outerTpl2obj(tplIterator.next())
      }
    }
    )
  }

  final override def getObjList(implicit conn: Conn): Future[List[Obj]] = {
