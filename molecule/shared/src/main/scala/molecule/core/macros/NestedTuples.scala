...
trait NestedTuples[Obj, OuterTpl] extends jComparator[jList[AnyRef]] { self: Molecule_0[Obj, OuterTpl] =>
  ...
  final override def getIterable(implicit conn: Conn, ec: ExecutionContext): Future[Iterable[OuterTpl]] = get(conn, ec)
  ...
  /**
    ...
    * @return Iterable[Obj]
    */
  final override def getObjIterable(implicit conn: Conn, ec: ExecutionContext): Future[Iterable[Obj]] = {
  ...
  final override def getObjList(implicit conn: Conn, ec: ExecutionContext): Future[List[Obj]] = {
...
object NestedTuples {
  ...
  trait NestedTuples1[Obj, OuterTpl] extends NestedTuples[Obj, OuterTpl] { self: Molecule_0[Obj, OuterTpl] =>
    ...
    final override def get(implicit conn: Conn, ec: ExecutionContext): Future[List[OuterTpl]] = {
      conn.query(_model, _nestedQuery.get).map { data =>
        resetCastVars
