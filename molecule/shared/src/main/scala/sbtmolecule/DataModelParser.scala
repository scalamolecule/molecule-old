  ...
  def parse: Model = {
    ...
    lines collect {
      case r"@InOut\((\d+)$in, (\d+)$out\)" => (in.toInt, out.toInt) match {
        case (i: Int, _) if i < 0 || i > 3  => throw DataModelException(s"Input arity in '$dataModelFileName' was $in. It should be in the range 0-3")
        case (_, o: Int) if o < 1 || o > 22 => throw DataModelException(s"Output arity of '$dataModelFileName' was $out. It should be in the range 1-22")
        case (i: Int, o: Int)               => (i, o)
      }
    } match {
      case Nil           => throw DataModelException(
      ...
      case h :: t :: Nil => throw DataModelException(
        """
    ...
    // Check domain name
    lines.collectFirst {
      case r"class (.*)${dmn}DataModel"        => throw DataModelException(s"Can't use class as data model container in $dataModelFileName. Please use an object:\nobject ${dmn}DataModel { ...")
      case r"class (.*)${dmn}DataModel \{"     => throw DataModelException(s"Can't use class as data model container in $dataModelFileName. Please use an object:\nobject ${dmn}DataModel { ...")
      case r"class (.*)${dmn}DataModel \{ *\}" => throw DataModelException(s"Can't use class as data model container in $dataModelFileName. Please use an object:\nobject ${dmn}DataModel { ...")
      case r"trait (.*)${dmn}DataModel"        => throw DataModelException(s"Can't use trait as data model container in $dataModelFileName. Please use an object:\nobject ${dmn}DataModel { ...")
      case r"trait (.*)${dmn}DataModel \{"     => throw DataModelException(s"Can't use trait as data model container in $dataModelFileName. Please use an object:\nobject ${dmn}DataModel { ...")
      case r"trait (.*)${dmn}DataModel \{ *\}" => throw DataModelException(s"Can't use trait as data model container in $dataModelFileName. Please use an object:\nobject ${dmn}DataModel { ...")
    }
    ...
    lines.collect {
      ...
      case Nil                      => throw DataModelException("Couldn't find data model object <domain>DataModel in " + dataModelFileName)
    ...
    val dataModel: Model = lines.zipWithIndex.foldLeft(0, "", Model("", -1, -1, "", "", "", Seq())) {
      case ((emptyLine, cmt, d), (line, i)) => line.trim match {
        ...
        case r"import molecule\.core\.data\.model\.(.*)$t"     => throw DataModelException(s"Data model api in $dataModelFileName (line ${i + 1}) should be imported with `import molecule.core.data.model._`")
  ...
  def markBidrectionalEdgeProperties(nss: Seq[Namespace]): Seq[Namespace] = nss.map { ns =>
    ...
    if (isEdge) {
      val newAttrs: Seq[DefAttr] = ns.attrs.map {
        ...
        case Ref(attr, _, _, _, _, _, _, Some(bi), _, _) if bi.substring(6, 10) != "Prop" => throw DataModelException(
