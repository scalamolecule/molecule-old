...
private[molecule] trait PackOptNestedTypes extends PackBase with Helpers {
  ...
  protected lazy val packOptNestedOneRefAttr = (sb: StringBuffer, it: jIterator[_]) => {
    it.next match {
      case l: jLong      => add(sb, l.toString)
      case m: jMap[_, _] => add(sb, m.values.iterator.next.toString)
    }
  }
  ...
  protected lazy val packOptNestedOptOneString = (sb: StringBuffer, it: jIterator[_]) => {
    it.next match {
      case "__none__" | null =>
      case m: jMap[_, _]     => add(sb, m.values.iterator.next.asInstanceOf[String])
  ...
  protected lazy val packOptNestedOptOneDate = (sb: StringBuffer, it: jIterator[_]) => {
    it.next match {
      case "__none__" | null => end(sb)
      case m: jMap[_, _]     => add(sb, date2str(m.values.iterator.next.asInstanceOf[Date]))
  ...
  protected lazy val packOptNestedOptOneEnum = (sb: StringBuffer, it: jIterator[_]) => {
    it.next match {
      case "__none__" | null => end(sb)
      case m: jMap[_, _]     => m.values.iterator.next match {
        case kw: Keyword   => add(sb, kw.getName)
        case m: jMap[_, _] => add(sb, m.values.iterator.next.asInstanceOf[Keyword].getName)
      }
      case s: String         => add(sb, s)
  ...
  protected lazy val packOptNestedManyString = (sb: StringBuffer, it: jIterator[_]) => {
    // Can be jList or jSet
    it.next.asInstanceOf[jIterable[_]].forEach { v =>
      add(sb, v.toString)
      end(sb)
    }
    end(sb)
  ...
  protected lazy val packOptNestedManyDate = (sb: StringBuffer, it: jIterator[_]) => {
    it.next.asInstanceOf[jIterable[_]].forEach(v => add(sb, date2str(v.asInstanceOf[Date])))
    end(sb)
  }

  protected lazy val packOptNestedManyEnum = (sb: StringBuffer, it: jIterator[_]) => {
    it.next.asInstanceOf[jIterable[_]].forEach {
      case m: jMap[_, _] => add(sb, m.values.iterator.next.asInstanceOf[Keyword].getName)
      case s: String     => add(sb, s)
    }
    end(sb)
  ...
  protected lazy val packOptNestedMany_ = (sb: StringBuffer, it: jIterator[_]) => {
    it.next.asInstanceOf[jIterable[_]].forEach(v => add(sb, v.toString))
    end(sb)
  }
  ...
  protected lazy val packOptNestedOptManyString = (sb: StringBuffer, it: jIterator[_]) => {
    it.next match {
      case "__none__" | null =>
      case vs: jList[_]      => vs.forEach { v =>
        add(sb, v.toString)
        end(sb)
      }
      case vs: jMap[_, _]    => vs.values.iterator.next.asInstanceOf[jList[_]].forEach { v =>
        add(sb, v.toString)
        end(sb)
      }
    }
  ...
  protected lazy val packOptNestedOptManyDate = (sb: StringBuffer, it: jIterator[_]) => {
    it.next match {
      case "__none__" | null =>
      case vs: jList[_]      => vs.forEach(v => add(sb, date2str(v.asInstanceOf[Date])))
      case vs: jMap[_, _]    => vs.values.iterator.next.asInstanceOf[jList[_]].forEach(v =>
  ...
  protected lazy val packOptNestedOptMany_ = (sb: StringBuffer, it: jIterator[_]) => {
    it.next match {
      case "__none__" | null =>
      case vs: jList[_]      => vs.forEach(v => add(sb, v.toString))
      case vs: jMap[_, _]    => vs.values.iterator.next.asInstanceOf[jList[_]].forEach(v => add(sb, v.toString))
    }
    end(sb)
  }
  ...
  protected lazy val packOptNestedMapString = (sb: StringBuffer, it: jIterator[_]) => {
    val vs = it.next.asInstanceOf[jIterable[_]].iterator
    while (vs.hasNext) {
