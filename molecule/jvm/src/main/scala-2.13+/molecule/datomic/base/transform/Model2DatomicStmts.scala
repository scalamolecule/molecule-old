...
case class Model2DatomicStmts(conn: Conn, model: Model) extends Model2Statements with Helpers {
  ...
  private def valueStmts(
    ...
    def default(card: Int): Iterable[Statement] = arg match {
      ...
      case Eq(newValues0) =>
        ...
        val curValues       = if (e == "datomic.tx") Nil else attrValues(e, a)
        val newValueStrings = newValues.map(_.toString)
        val curValueStrings = curValues.map(_.toString)
  ...
  private def lastE(stmts: Seq[Statement], attr: String, forcedE: Any, bi: GenericValue, composite: Any = "e"): Any = {
    bi match {
  ...
  private def lastV(stmts: Seq[Statement], attr: String, forcedE: Any = 0): Any = {
    if (forcedE != 0)
      forcedE
    else if (stmts.isEmpty)
      tempId(attr)
  ...
  def insertStmts(dataRows: Iterable[Seq[Any]]): Seq[Statement] = {
    ...
    val dataStmtss: Seq[Statement]     = dataRows.toSeq.flatMap(resolveStmts(genericStmts, _, 0))
    val txId                           = tempId("tx")
    val txStmtss  : Seq[Statement]     = genericTxStmts.foldLeft(Seq.empty[Statement]) {
