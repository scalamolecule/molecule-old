...
case class DatomicEntity_Peer(
  ...
  private[molecule] def toScala(
    ...
    case e: datomic.Entity if depth < maxDepth && tpe == "Map" =>
      DatomicEntity_Peer(e, conn, e.get(":db/id")).asMap(depth + 1, maxDepth)

    case e: datomic.Entity if depth < maxDepth && tpe == "List" =>
      DatomicEntity_Peer(e, conn, e.get(":db/id")).asList(depth + 1, maxDepth)

    case e: datomic.Entity =>
      e.get(":db/id").asInstanceOf[Long]

    case set: clojure.lang.PersistentHashSet =>
      sortList(set.asScala.toList.map(toScala(_, depth, maxDepth, tpe)))
